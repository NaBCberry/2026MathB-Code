...
- **完备性测站**（地图上的"测站/覆盖"层）：第三、四题**为保证"全部清除"必须巡访的
  ...
  判"处处无信号 ⇒ 不存在"）；第四题 25 站（内层六边形格 13 + 半径 1900 m 外环 12，
  因为定向源的 `no_signal` 可能只是"在盲区"，判据要升级成"包围"，必须把测站铺到
  区域**外面**）。切换"离线题目"下拉时地图上的这一层会立刻跟着换；区域外的外环站用
  **橙色**区分（图例里也写了）。汇总卡片里会显示本局实际用的是哪一套；如果 trace 里
  带了算法**实际巡访**的站点（`trace_client` 在开跑时写入，队友的 29/35 站版本也能带），
  就按实际那一套画，并标出算法名。
...
- **SSE 推送**（`/api/stream`）：服务端每 50 ms 看一眼 trace 文件有没有新增，
  一有新动作**立刻推**给浏览器（实测文件写入 → 页面收到 ≈20–60 ms），
  没有变化就闭嘴（每 2 s 一个心跳）。运行中页面跟着机器狗的动作走，不再有 250 ms 的固定延迟。
- **轮询兜底**（`/api/poll`）：推送不可用时自动退回轮询，闸门状态与轨迹增量**一次请求拿全**
  （原来每轮两个请求），运行中 120 ms 一轮、空闲 700 ms 一轮。
- **渲染每帧合并**：无论数据来得多么密集，卡片/地图/表格都合并到同一个动画帧里重画一次，
  表格只把新行 `insert` 到表头，不会整表重刷。
- "自动刷新"勾选框现在就是整条数据通道的开关；关掉即暂停，重新勾上会从**当前进度**接着收，
  既不重复也不漏。顶部状态栏会显示当前是"实时推送"还是"轮询"。
- **不重复、不错位**：动作按序号 `seq` 去重（同一批事件被送两次也只画一行）；
  整表重建时会作废"还没画出来的增量行"，所以换案例/重跑同名案例的瞬间不会出现
