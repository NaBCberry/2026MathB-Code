...
> 面向"机器狗"程序开发者的实操手册
> 编写日期：2026-09-10
> 姊妹文档：**`Python速成_面向C++程序员.md`**（Python 速成 + 速查表 + 编码辅助手册）
...
├── config.json              # 接口地址、物理常数（★ 不放参赛队号）
├── sim_client.py            # ★ 通信客户端（4 条指令，无策略）
├── robot.py                 # ★ 程序入口：解析参数 → 套录制/调试外壳 → 跑算法
├── algorithms\              # ★ 算法本体 + 注册表（一个 .py 一个算法）
│   ├── p3_baseline.py       #   第三题初版算法：搜索 / 定位 / 清除
│   ├── base.py              #   算法接口约定（SPEC + build）
│   ├── _template.py         #   新算法模板（复制改名即可）
│   └── README.md            #   怎么选算法 / 加算法 / 改算法
├── trace_client.py          # 行为轨迹录制壳（供网页可视化）
├── step_gate.py             # 调试闸门：每步延迟 / 断点单步
├── webui.py / webui.html    # ★ 可视化网页
├── archived\                # 归档：mock_arena.py、probe_simulator.py、robot_v1.py
├── docs\
│   ├── 编程手册.md                    # ★ 本文档：协议与接口速查
│   └── Python速成_面向C++程序员.md     # ★ Python 速成 + 速查表 + 编码辅助
├── logs/                    # 客户端行为日志（JSONL）
└── _extracted/              # docs 下 PDF/DOCX 提取出的纯文本（便于检索）
```

> 算法代码一律写在 `algorithms/` 下；`robot.py` 只是入口，不含策略。
> 连不上模拟器时先跑 `python archived/probe_simulator.py --robot-id <队号>`。

### 1.2 参赛队号：从命令行传入

参赛队号**不写进任何文件**，一律用 `--robot-id` 传参（避免被提交进版本库）：

```powershell
python probe_simulator.py --robot-id 2026xxxxxxxx
python robot.py           --robot-id 2026xxxxxxxx
```

三个要求：

1. 必须与**模拟器当前登录的参赛队号逐字节一致**（UTF-8 长度 1–64 字节）；
2. 不能含控制字符或不可见格式字符；
3. 所有脚本的 `--robot-id` 都是**必填**，漏传直接报错，不会静默用错号。

F5 调试时 VS Code 会弹框让你输入（`${input:robotId}`，见 1.4），同样不落盘。
`config.json` 里从此只有物理常数与接口地址。
...
> `cwd` 固定为工作区根目录，`logs/` 等相对路径能正确定位。
> 首次按 F5 会弹框让你填参赛队号（`${input:robotId}`），填一次即可，不写进任何文件。
...
> **题目没有给出机器狗的统一探测半径。** 给的是**每个干扰源各自的有效接收半径**
> $R \in [1000, 1500]\ \mathrm{m}$（由该源发射功率与测向机灵敏度共同决定，**接口不返回，需自行探测推断**）。

在检测点 $S$ 对频道 $c$ 的干扰源 $G$ 执行 `/measure`，记 $d = \lVert S - G \rVert$：

| 条件 | 返回值 | 说明 |
|---|---|---|
| $5 < d \le R_G$，且（$G$ 全向，或 $S$ 在其 $180^\circ$ 扇区内） | `direction` + `svd_deg` | 正常测向 |
| $d \le 5$，且 $S$ 在该源覆盖范围内 | `near` | **内层盲区**，信号过强，无示向度 |
| $d > R_G$ | `no_signal` | 超距 |
| $G$ 是定向源且 $S$ 在其覆盖扇区外 | `no_signal` | 角度遮挡 |
| 该频道无源 / 已被清除 | `no_signal` | — |

**径向分区示意**（以干扰源 $G$ 为中心）：

```
   0 ─── 5 m ────────────────── R_G (1000~1500 m) ──────────→
   │  near  │        可测向（direction）        │ no_signal │
   └─ 盲区 ─┴───────────────────────────────────┴───────────┘
```

关键点：

- **机器狗侧无角度限制**（探测等效全向）；角度限制来自**定向源自身的 $180^\circ$ 覆盖**。
- 坐标超出 1800 m 圆域也**不是**软约束：模拟器仍按提交位置计算，只是容易 `no_signal`。
- 清除与探测不同：`/clear` 只看 $d \le 20\ \mathrm{m}$，**不看覆盖角度**。
...
sim = SimulatorClient(
    robot_id="T2026xxxxxx",   # 必填，且必须与模拟器登录队号逐字节一致
    base_url="http://127.0.0.1:2026",
    timeout=5.0,          # 单次 HTTP 超时
    retries=3,            # 仅对"连不上 / 响应不是 JSON"重试，且复用原 request_id
    log_dir="logs",       # 写 JSONL 行为日志；None 则不写
    verbose=True,         # 控制台回显每条响应
)

info = sim.enter()                              # → dict
info["remaining_real_duration_s"]               # 本局现实可用秒数

r = sim.measure(300, 400, 1)                    # → dict
r["measure_result"]                             # 'direction' | 'near' | 'no_signal'
r["svd_deg"]                                    # 仅 direction 时存在，含 ±1° 误差
r["virtual_time_s"]                             # 本次动作后的虚拟时刻

c = sim.clear(300, 0, 3)                        # → dict
c["clear_result"]                               # 'success' | 'no_target_in_range'

sim.exit()["exit_reason"]                       # 'user_exit'
sim.close()
```

**设计取舍**：客户端是**薄封装**，只做"发送 + 校验 + 留痕"，返回响应 `dict`，
**不保存任何世界状态**。`position`、`channel`、`virtual_time_s`、剩余时间等属于**策略层**，
请在 `robot.py` 里自行维护 —— 其中 `/clear` 不切频道、`accepted=false` 的 `virtual_time_s=0`
不是当前时刻，是最容易记错的两处。
...
| 情况 | 行为 |
|---|---|
| 连不上 / 响应不是 JSON | 按协议**原样重发**（`request_id` 不变），`retries` 次后抛 `OSError` / `ValueError` |
| `HTTP ≠ 200` | 抛 `RuntimeError`（含状态码与响应体前 300 字符） |
| `accepted=false` | 抛 `RuntimeError`（字段拼错 / `robot_id` 不匹配 / 状态不对，属程序缺陷） |

`with` 块退出时会补一次 `/exit`（失败则忽略）并关闭日志文件，避免异常时白烧一次正式测试机会。
...
```json
{"event":"response","path":"/measure","http_status":200,"request_id":"measure-3","response":{"accepted":true,"virtual_time_s":111.0,"measure_result":"direction","svd_deg":123.45}}
{"event":"retry","path":"/measure","request_id":"measure-4","attempt":1,"error":"ConnectionResetError(10054, ...)"}
```
...
2. `robot_id` 用当前登录的参赛队号，通过 `--robot-id` 传参，**不要写进文件**。
...
| 以为机器狗有固定探测半径 | 有效接收半径 $R \in [1000,1500]$ 是**每个源各自**的属性，接口不返回 | 用多点"刚出现/刚消失信号"的边界二分反推 $R$ |
| 以为要指定天线朝向 | `/measure` 只有 `position` + `channel`，没有朝向字段；"不断调整天线朝向"的代价已固定折算为 5 s 检测耗时（`B题.pdf` 附录 2(5)） | 只提交位置与频道；乱加字段会得到 `200 + accepted=false` |
| 把参赛队号写进 `config.json` 并提交 | 队号会永久留在版本库历史里 | 一律用 `--robot-id` 传参 |
...
`->` 后面写的是**返回值类型**。它是**给自己和 Pylance 看的注释**，Python 运行时**完全不检查**。
...
| 代码 | 读法 |
|---|---|
| `def parse_args() -> argparse.Namespace:` | 返回一个 `argparse.Namespace` 对象 |
| `def run(sim: SimulatorClient) -> None:` | 参数 `sim` 是 `SimulatorClient`；**函数不返回任何东西** |
| `def main() -> None:` | 不返回东西，只是执行 |
| `def enter(self) -> dict:` | 返回字典 |
| `def _post(self, path: str, payload: dict) -> tuple[int, str]:` | 返回二元组 `(状态码, 响应体)` |
| `def __init__(...) -> None:` | 构造函数永远是 `-> None` |

### A.3 `xxx.xxx` 是什么

就是**「模块.名字」的限定名**（dotted name）：

- `argparse.Namespace` = `argparse` 模块里的 `Namespace` 类
- `pathlib.Path` = `pathlib` 模块里的 `Path` 类

`SimulatorClient` 不用写前缀，是因为它已经被 `from sim_client import SimulatorClient` 引到当前命名空间了。
...
|---|---|---|
| `-> None` | 不返回值 | `void` |
| `-> int` `-> float` `-> str` `-> bool` | 基本类型 | `int` / `double` / `std::string` / `bool` |
| `-> dict` | 字典 | `std::unordered_map<std::string, T>` |
| `-> list[float]` | 列表 | `std::vector<double>` |
| `-> tuple[bool, str]` | **定长**二元组 | `std::tuple<bool, std::string>` |
| `-> str \| None` | 可能是 `str` 也可能是 `None` | `std::optional<std::string>` |
| `-> dict[str, Any]` | 键 `str`、值任意 | `std::map<std::string, std::any>` |
| `sim: SimulatorClient` | 参数标注 | 函数签名里的参数类型 |

`|` 读作「**或者**」（联合类型，PEP 604）：`str | Path | None` = 这三者中的任意一个。
...
```python
def f() -> int:
    return "我偏要返回字符串"     # 运行完全正常，只是 Pylance 画黄线
```

所以 —— **写了更好**（IDE 能自动补全、能提前发现拼写错误、别人读代码一眼知道返回什么），**不写也能跑**。

### A.6 `from __future__ import annotations` 是干什么的

`robot.py`、`sim_client.py` 第 10 行都有这句。作用是让所有标注**以字符串形式保存**，而不是定义时立刻求值。好处：

```python
from __future__ import annotations

class Node:
    def next(self) -> Node:      # 没有这句会报错：此时 Node 还没定义完
        ...
```

顺带让模块导入快一丁点（不去解析标注对象）。

### A.7 实操建议

- 返回值有用 → 写具体类型（`-> dict`、`-> float`、`-> tuple[float, float]`）
- 只是执行动作 → 写 `-> None`
- 一时想不清 → **先不写**，能跑，等接口稳定了再补

---

## 附录 B　Python 函数定义括号里怎么写（对照 C++）
...
**最核心的一条差异**：C++ 把**类型写在名字前面**，Python 把**类型写在名字后面（而且可以不写）**。
...
|---|---|---|
| 无参数 | `void f();` | `def f():` |
| 带参数 | `int f(int a, int b)` | `def f(a, b):` |
| 返回值类型 | 写在函数名**前**：`int f()` | 写在括号**后**：`def f() -> int:` |
| 默认参数 | `void f(int a, int b = 0)` | `def f(a, b=0):` |
| 引用传参 | `void f(std::vector<int>& v)` | 不需要，直接 `def f(v):` |
| 常引用 | `void f(const std::string& s)` | 没有这个概念，`def f(s):` |
| 可变参数 | `template<class... Ts> void f(Ts... xs)` | `def f(*args):` → `args` 是 `tuple` |
| 命名参数 | 无（C++20 的结构体指定初始化是最近似的） | 调用时可写 `f(a=1, b=2)` |
| 重载 | `int f(int); int f(double);` | **不支持**，后一个 `def` 直接覆盖前一个 |
| 函数原型/前向声明 | 需要 `.h` 里声明 | 不需要，定义即声明 |
| 函数指针 | `int (*p)(int) = &f;` | 直接 `p = f`（函数本身就是对象） |
| 成员函数 | `class A { int f(); };` | `class A: def f(self):` |
| 静态成员函数 | `static int f();` | `@staticmethod` |
| 构造函数 | `A(int x) : x_(x) {}` | `def __init__(self, x):` |
| `const` 成员函数 | `int f() const;` | 无对应；约定用 `_` 前缀表示私有 |
| 花括号 `{}` | 包住函数体 | 用**冒号 `:` + 缩进** |
...
class Dog {
    int x;
public:
    int bark() { return x; }        // this 隐式存在
};
```

```python
class Dog:
    def bark(self):                 # self 必须显式写在第一个参数
        return self.x
```

- 写 `dog.bark()` 时，Python **自动**把 `dog` 传给 `self`（≈ C++ 的 `this`，但必须自己写出来）
- `@staticmethod` ≈ C++ 静态成员函数
- `@classmethod` 的第一个参数是**类本身**（习惯叫 `cls`），C++ 没有直接对应物

### B.4 默认参数的坑（C++ 没有的问题）

Python 的默认值**在函数定义时求值一次**，然后被所有调用共享：
...
C++ 的默认参数是**每次调用时求值**的表达式，所以 `void f(std::vector<int> v = {});` 没这个坑。

### B.5 `*` 和 `/`：强制命名传参

```python
def SimulatorClient(robot_id, base_url=BASE_URL, *, timeout=5.0, log_dir=None): ...
#                                                  ↑ 这之后的参数必须写成 名字=值
```

```python
SimulatorClient("2026xxxxxxxx")                      # ✅
SimulatorClient("2026xxxxxxxx", timeout=3)            # ✅
SimulatorClient("2026xxxxxxxx", 5.0, 3)               # ❌ TypeError
```

C++ 没有这个机制 —— 只能靠在参数名上下功夫。反向的 `/` 表示「这**之前**的参数只能按位置传」。

### B.6 一个真实的实例对照

`sim_client.py` 里客户端的构造函数：
...
等价的 C++（类型是强制的，所以不写 `-> None`）：

```cpp
class SimulatorClient {
public:
    SimulatorClient(std::string robot_id,
                    std::string base_url = BASE_URL,
                    double timeout = 5.0,                     // C++ 无法强制"必须写名字"
                    int retries = 3,
...
1. C++ 类型前置且必写；Python 类型后置且可省
2. C++ 没有「命名传参」约束，Python 的 `*` 之后只能写 `名字=值`
3. Python 的 `str | Path | None` ≈ C++ 的 `std::optional<std::filesystem::path>`

### B.7 一句话口诀

```
def 名字(参数1, 参数2=默认值, *args, 命名参数=默认值, **kwargs) -> 返回类型:
    ↑         ↑                                            ↑
  声明关键字  类型可写可不写："名字: 类型"                不返回东西就写 None
```

**括号里跟 C++ 一样就是参数列表**，唯一的区别是 —— **类型写在名字后面，而且可以不写**。

---

## 附录 C　Python 与 C++ 的其它常见差异（速查）

| 事项 | C++ | Python |
|---|---|---|
| 语句结束 | `;` | 换行 |
| 代码块 | `{ }` | 冒号 + **缩进**（4 空格） |
| 变量声明 | `int x = 0;` | `x = 0`（不声明类型） |
| 常量 | `const int N = 20;` | `N = 20`（约定全大写表示"别改"） |
| 整除 | `7 / 2 == 3` | `7 // 2 == 3`，而 `7 / 2 == 3.5` |
| 数组越界 | UB | 抛 `IndexError` |
| 字符串长度 | `s.size()` | `len(s)` |
| 遍历 | `for (auto& v : xs)` | `for v in xs:` |
| 字典遍历 | `for (auto& [k,v] : m)` | `for k, v in m.items():` |
| 空值 | `nullptr` | `None` |
| 空指针访问 | UB / 崩 | `AttributeError` 异常（可 `try`） |
| 布尔运算 | `&&` `\|\|` `!` | `and` `or` `not`（也可用 `&` `\|` `~` 但语义不同） |
| 匿名函数 | `[](int x){ return x*2; }` | `lambda x: x * 2` |
| 命名空间 | `std::` | 模块 + `import` |
| 编译 | 需要 | 不需要，`python xxx.py` 直接跑 |
| 类型错误 | 编译期报错 | 运行到那行才报 |
