...
```python
def build(sim, params=None):
    import robot
    robot.RING_R = (params or {}).get("ring_r", 1600.0)   # 只改参数
    return robot.InterferenceHunter(sim, verbose=False)
```
...
用"改 `robot.py` 模块级常量"实现的，对照表在 `algorithms/p3_baseline.py` 的
`PARAM_CONSTANTS`：
...
第三题那套的实现仍在 `robot.py`（`InterferenceHunter`），对照表：
...
`max_refine` …）要跟 `robot.py` 的常量名对得上；对不上就在 `build()` 的 `mapping`
里补一条。
