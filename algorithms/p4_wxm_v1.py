...
实现 ``InterferenceHunter``（本文件即 ``algorithms/p4_wxm_v1.py``）。网页可视化见
``webui.py``。
...
1. 覆盖布站（35 站）
   原点 + 半径 900 m × 8 点 + 半径 1400 m × 10 点 + 半径 1900 m × 16 点。
   1900 m 环在目标区域(1800 m)之外，专门包围「贴着边界、朝外/切向发射」的定向源。
   该布站满足：**对场内任意位置、任意指向的定向源，必有一个测站同时满足
   「距源 ≤ 1000 m 且落在源的 180° 扇形内」**（已按 40 m 网格 × 5° 指向数值穷举验证，
   失败数 0）。于是：
       (a) 任意干扰源（全向或定向）必被至少一个测站测到；
       (b) 扫完 35 站仍「全程无信号」的频道必不存在——停止判据恢复，不猜总数。
...
3. 定向鲁棒的精化逼近
   (a) 只有一条示向度时，第二点取「沿示向 +750 m 再垂直偏置 ±600 m」的两侧候选：
       该点到源的最近距离 < 960 m（对任意 5–1500 m 源距都 < 1000 m），保证收得到；
       且两侧候选正好互成 180° 方向，定向源必对其中一侧可见（180° 扇形必含两个
       相反方向之一）。哪一侧先测到示向度就采信哪一侧。
...
4. 滚动巡访
   待办池 = {未去的 35 个测站} ∪ {待补第二条示向度的频道} ∪ {待清除目标}，每步对当前
   待办点集做一次滚动 TSP 取最近点——探测、定位、清除共用同一条回路。

【与 Q3 的关键差异小结】
   - 探测环从 1+6 站升级为 1+8+10+16=35 站（含 1900 m 外环）；
   - 停止判据不再是「no_signal 覆盖自检」，而是「35 站扫完 + 全程无信号 ⇒ 不存在」；
...
# ------------------------------------------------------------------ 策略参数
# 覆盖布站：保证「任意定向源必被探测」的三环 + 中心（含 1900 m 外环）
SURVEY_RINGS = [(900.0, 8), (1400.0, 10), (1900.0, 16)]
...
class InterferenceHunter:
    ...
    def survey_stations() -> list[tuple[float, float]]:
        """35 站覆盖布站（含 1900 m 外环），保证任意定向源必被探测。

        顺序：中心 → 内环 → 中环 → 外环，每环按角序排列，交给滚动 TSP 再优化。
        """
        pts = [(0.0, 0.0)]
    ...
    def clean_sweep(self, x: float | None = None, y: float | None = None) -> None:
        ...
        与 Q3 不同：Q4 的停止判据是「35 站扫完 + 全程无信号 ⇒ 不存在」，因此这里
...
SPEC = AlgorithmSpec(
    ...
    name="第四题定向鲁棒算法",
    problem="问题4",
    summary="定向覆盖布站(35站) → 交会定位 → 定向鲁棒精化 → 滚动巡访清除",
    description=(
        "【探测】原点 + 900m×8 + 1400m×10 + 1900m×16 共 35 站。1900m 外环包围贴着边界"
        "朝外/切向发射的定向源；该布站保证「场内任意位置、任意指向的定向源必被某站以"
        "≤1000m 且在扇形内探测到」，故「扫完 35 站仍全程无信号 ⇒ 频道必不存在」，停止"
        "判据不再依赖猜总数。\n"
        "【定位】扇形内场均匀，示向度仍指向源，故交会定位与 Q3 相同：±1° 楔形求交，"
        "再与「被检测到 ⇒ 距站 ≤1500m」圆盘、1800m 圆域求交，最小外接圆半径 R_c=D/2 "
        "是保证性误差界。\n"
        "【定向鲁棒精化】单示向度时第二点取「沿示向 +750m、垂直 ±600m」两侧候选，两侧互成"
        "180°，定向源必对一侧可见；精化补测 no_signal 解释为「在扇形外」，换对侧继续。"
        "R_c ≤ 20m 后取圆心清除，清除与定向方向无关，一次命中可保证。\n"
    ...
    entry="algorithms/p4_wxm_v1.py: InterferenceHunter",
)
